...
# On va mapper les niveaux de pollution
niveau_map = {
    "pollution faible": 10,
...
# On va supprimer les lignes sans pollution connue
df = df.dropna(subset=["pollution_num"])

# On va créer un graphe avec les stations comme nœuds
G = nx.Graph()
for _, row in df.iterrows():
    station = row["Nom de la Station"]
    G.add_node(station, pollution=row["pollution_num"])

# On va lire les connexions entre stations
...
# Entrée de utilisateur et la validation
