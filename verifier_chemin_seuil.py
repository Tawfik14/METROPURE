...
#  Créer le graphe avec pollution par station
G = nx.Graph()
for _, row in df.iterrows():
    station = row["Nom de la Station"]
    G.add_node(station, pollution=row["pollution_num"])

# On va ajouter les connexions depuis graphe_metro.csv 
...
# On va filtrer les arêtes selon la pollution max autorisée 
G_filtre = nx.Graph()
for u, v, data in G.edges(data=True):
    if data["pollution"] <= seuil_pollution:
        G_filtre.add_edge(u, v, pollution=data["pollution"])

# On va afficher les résultats
