...
if uploaded_file:
    ...
    temps_max_input = st.text_input("Temps maximum autorisé (minutes)", value="0.00")

    if st.button("Calculer le chemin optimal"):
        if not station_depart.strip() or not station_arrivee.strip() or not temps_max_input.strip():
            st.warning(" Merci de remplir tous les champs pour lancer le calcul.")
        else:
            try:
                temps_max = float(temps_max_input)
                chemin_input = f"{station_depart.strip()}\n{station_arrivee.strip()}\n{temps_max}\n"
                result = subprocess.run(
                ...
                st.error(" Merci de saisir un temps maximum valide (nombre décimal ou entier).")
